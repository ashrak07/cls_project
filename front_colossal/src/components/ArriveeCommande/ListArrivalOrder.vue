<template>
  <v-container class="py-6">
    <v-card elevation="3">
      <v-card-title class="text-h6">Liste des Arrivees</v-card-title>
    ...
    <v-dialog v-model="dialogView" max-width="900">
      <v-card>
        <v-card-title class="text-h6">Details de l'arrivee</v-card-title>
    ...
    <v-dialog v-model="dialogEdit" max-width="720">
      <v-card>
        <v-card-title class="text-h6">Editer l'arrivee</v-card-title>
        <v-card-text>
          <v-form ref="editForm" v-model="editValid" lazy-validation>
            <v-row>
              <v-col cols="12" md="4">
                <v-text-field v-model="editArrival.reference" label="Reference" variant="outlined" />
              </v-col>
              <v-col cols="12" md="4">
            ...
            <v-row>
              ...
              <v-col cols="12" md="3">
                <v-text-field
                  v-model.number="editArrival.tva"
                  label="TVA (%)"
                  type="number"
                  step="0.1"
              ...
              <v-col cols="12" md="3">
                <v-text-field
                  v-model.number="editArrival.frais"
                  label="Frais"
                  type="number"
                  step="0.01"
                  :rules="[rules.nonNegative]"
    ...
    <v-dialog v-model="dialogDelete" max-width="520">
      <v-card>
        <v-card-title class="text-h6">Supprimer l'arrivee</v-card-title>
        <v-card-text>
          Confirmer la suppression de la reference <strong>{{ selected?.reference || selected?.id }}</strong> ?
        </v-card-text>
        <v-card-actions>
          <v-spacer />
...
const arrivees = ref([
  ...
  },
  {
    ...
    fournisseurNom: "Import Ocean Indien",
    date: "2025-02-10",
    magasin: "Annexe 1",
    mode: "Transfert",
    tva: 0,
    frais: 0,
    lignes: [{ produit: "Mixeur", cout: 80000, quantite: 3 }],
  },
  {
    id: 3,
    ...
    tva: 20,
    frais: 12000,
    lignes: [{ produit: "Casque Bluetooth", cout: 98000, quantite: 5 }],
  },
]);
...
const rules = {
  ...
  nonNegative: (v) => Number(v) >= 0 || "Doit etre >= 0",
};
...
const saveEdit = async () => {
  ...
  if (!isValid) return;
  const idx = arrivees.value.findIndex((a) => a.id === editArrival.value.id);
  if (idx !== -1)
    arrivees.value[idx] = { ...arrivees.value[idx], ...editArrival.value };
  snackbarMessage.value = "Arrivee mise a jour";
  snackbar.value = true;
...
const confirmDelete = () => {
  ...
  arrivees.value = arrivees.value.filter((a) => a.id !== selected.value.id);
  snackbarMessage.value = "Arrivee supprimee";
  snackbar.value = true;
